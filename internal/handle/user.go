...
func (*User) GetInfo(c *gin.Context){
	...
	//从redis中获取用户点赞信息
	var err error
	userInfo.ArticleLikeSet, err = rdb.SMembers(rdbctx,g.ARTICLE_LIKE_COUNT + strconv.Itoa(auth.ID)).Result()
	if err != nil {
		ReturnError(c,g.ErrRedisOp,err)
		return
	}
	userInfo.CommentLikeSet, err = rdb.SMembers(rdbctx,g.COMMENT_LIKE_COUNT + strconv.Itoa(auth.ID)).Result()
	if err != nil {
		ReturnError(c,g.ErrRedisOp,err)
		return
	}
...
func (*User)UpdateCurrent (c *gin.Context){
	...
	auth,_ := CurrentUserAuth(c)
	err := model.UpdateUserInfo(GetDB(c),auth.ID,req.Nickname,req.Avatar,req.Intro,req.Website)
	if err !=nil {
...
func (*User) Update(c *gin.Context){
	...
	if err := model.UpdateUserNicknameAndRole(GetDB(c),auth.ID,req.Nickname,req.RoleIds); err!= nil {
		ReturnError(c,g.ErrDbOp,err)
		return
	}
	ReturnSuccess(c,nil)
...
func (*User) UpdateCurrentPasswordReq (c *gin.Context){
	...
	// 先判断旧密码是否正确
	auth,_ := CurrentUserAuth(c)
	if !utils.BcrypyCheck(req.OldPassword,auth.Password){
...
func (*User) ForceOffline(c *gin.Context){
	...
	// 再添加到离线用户的键值
	rdb.SAdd(rdbctx,g.OFFLINE_USER+strconv.Itoa(uid),uid,time.Hour)
